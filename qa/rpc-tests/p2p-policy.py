#!/usr/bin/env python3
# Copyright (c) 2021-2022 The Voltaem Core developers
...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # a Voltaem Core node that behaves similar to mainnet policies
    ...
    def run_test(self):
        ...
        ### test constants ###
        koinu = Decimal("0.00000001")          # 1 Koinu expressed in VAEM
        ten = Decimal("10.0")                  # uniform 10 VAEM seed moneys
        ...
        # create a bunch of UTXO with seed money from the Voltaem Core wallet
        for i in range(20):
        ...
        # test legacy output of 1 VAEM output and 1 VAEM fee
    ...
    # spend seed money with a key not in the Voltaem Core wallet.
    def spend_utxo(self, output, expected_size, retries=0):
        # construct the transaction using Voltaem Core raw tx APIs
        ...
        # and send the tx from there rather than from Voltaem Core, to test
