...
/* 1.14.5: set the wallet's discard threshold to 1 VAEM because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 VAEM. Very network
...
/*
 * Voltaem: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Voltaem: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Voltaem: Get a fee targetting a specific transaction speed.
     */
    CAmount GetVoltaemPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Voltaem: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetVoltaemPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Voltaem dust fee.
     */
