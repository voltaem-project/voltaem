...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of Voltaem Core repository.
...
```bash
gpg: skipped "vaemtoshi": secret key not available
```
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/voltaem-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/voltaem-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/voltaem-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/voltaem/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [voltaem/gitian.sigs](https://github.com/voltaem/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of Voltaem Core*!**
