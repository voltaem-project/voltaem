...
You must install required dependencies to build a basic Voltaem daemon, optional dependencies may vary according to your requirements.
...
Voltaem Core's released binaries are built using the "depends system", which
...
**Required dependencies** :
```bash
sudo apt-get install build-essential libtool autotools-dev automake pkg-config libssl-dev libevent-dev bsdmainutils autoconf-archive autoconf m4
sudo apt-get install libboost-system-dev libboost-filesystem-dev libboost-chrono-dev libboost-program-options-dev libboost-test-dev libboost-thread-dev
```

**Optional dependencies** :
```bash
# Qt (required for voltaem-qt GUI)
...
According to installed dependencies, the following steps will compile `voltaemd`, `voltaem-cli` and `voltaem-qt`.
...
#### Enable Qt GUI
Create `voltaem-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Voltaem Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Voltaem Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Voltaem may be compiled in
...
#### Security
To help make your Voltaem installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Voltaem should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Voltaem Core.
