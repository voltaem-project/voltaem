...
This guide describes how to build voltaemd and command-line utilities on OpenBSD.
...
The default C++ compiler that comes with OpenBSD 5.9 is g++ 4.2. This version is old (from 2007), and is not able to compile the current version of Voltaem Core, primarily as it has no C++11 support, but even before there were issues. So here we will be installing a newer compiler.
...
Do not use `pkg_add boost`! The boost version installed thus is compiled using the `g++` compiler not `eg++`, which will result in a conflict between `/usr/local/lib/libestdc++.so.XX.0` and `/usr/lib/libstdc++.so.XX.0`, resulting in a test crash:
    ...
    test_voltaem:/usr/lib/libstdc++.so.57.0: /usr/local/lib/libestdc++.so.17.0 : WARNING: symbol(_ZN11__gnu_debug17_S_debug_me ssagesE) size mismatch, relink your program
    ...
    Segmentation fault (core dumped)

This makes it necessary to build boost, or at least the parts used by Voltaem Core, manually:

```
# Pick some path to install boost to, here we create a directory within the voltaem directory
BITCOIN_ROOT=$(pwd)
...
# Build w/ minimum configuration necessary for voltaem
...
```bash
# Pick some path to install BDB to, here we create a directory within the voltaem directory
BITCOIN_ROOT=$(pwd)
