...
This tutorial will help you to go through the basics to use Voltaem Core after you completed the [installation instructions](/INSTALL.md). You now have `voltaemd` or `voltaem-qt` executables available to run a node, and `voltaem-cli`/`voltaem-tx` tools to help you transact VAEM.
...
> ```console
> shibetoshi:~$ /path/to/voltaem-cli [arguments ...]
> ```

### Table of contents

1. [Starting a voltaem node](#starting-a-voltaem-node)
...
## Starting a Voltaem node

To start your node, you can run an headless server using `voltaemd`:
```console
shibetoshi:~$ voltaemd -daemon
```

Or you can use the Graphical User Interface (GUI), `voltaem-qt`:
```console
shibetoshi:~$ voltaem-qt
```
...
Your node is now running and starts with a *synchronization process* that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Voltaem network!

> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `voltaem-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `voltaem-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

Voltaem Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The Voltaem Core installation provides the `voltaem-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `20925`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ voltaem-cli help

#Get help for a specific command
shibetoshi:~$ voltaem-cli help COMMAND
```
...
To receive VAEM, you need an address that is securely derived from a private key through a series of automatic, cryptographic operations. The *address* can be shared with anyone to receive VAEM, but the *private key* is  sensitive information that allows anyone that knows it to spend the VAEM on the associated address.

By default, the Voltaem Core software will automatically create an address for you and securely store the private key in the wallet file.

You can list wallet addresses using `getaddressesbyaccount`:

```console
shibetoshi:~$ voltaem-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ voltaem-cli getnewaddress
...
```console
shibetoshi:~$ voltaem-cli backupwallet /path/of/wallet/backup
```

**Tip:** Voltaem addresses start with the letter `D`.

You now have two wallet addresses to share with other people to receive VAEM! Consider avoiding [address reuse](https://en.bitcoin.it/wiki/Address_reuse) for anonymity and security reasons.
...
```console
#Syntax
shibetoshi:~$ voltaem-cli getbalance "*" minconf
```
...
```console
#Syntax
shibetoshi:~$ voltaem-cli sendtoaddress address amount

#Example
shibetoshi:~$ voltaem-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ voltaem-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ voltaem-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ voltaem-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ voltaem-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `voltaem-cli decoderawtransaction` or `voltaem-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. Voltaem Core will automatically use the correct private key when spending UTXO known to the wallet.

```console
#Syntax
shibetoshi:~$ voltaem-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ voltaem-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ voltaem-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ voltaem-cli sendrawtransaction 
...
> **Note:** To be able to query transactions not related to your own wallet, like in this example, you will need to enable the `-txindex` option. This options requires the Voltaem Core software to re-index the entire blockchain, and can take up to several hours.

First, request the information about block 69:

```console
#Find block hash from his height
shibetoshi:~$ voltaem-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ voltaem-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ voltaem-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ voltaem-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `voltaemd -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ voltaemd -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `voltaem.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.voltaem` by default, or specify the file location with `-conf`.

**voltaem.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/voltaem.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the Voltaem chain, it is recommended to not use the main Voltaem network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `voltaemd -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `voltaem-cli`.

### Data directory

The data directory is the location where Voltaem Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.
...
| RPC      |   20925 |   44555 |   18332 |

To configure them use the `-port` and `-rpcport` parameters.

### Memory

Running Voltaem Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, Voltaem Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.
