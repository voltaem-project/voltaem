Voltaem Core version 1.14.7 is now available from:

  <https://github.com/voltaem/voltaem/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for Voltaem-Qt. Voltaem-Qt users on
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/voltaem/lists/voltaem-releases>


Compatibility
==============

Voltaem Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for Voltaem Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside Voltaem Qt, support for BIP-70 payment
...
inside Voltaem Qt. By default, incoming BIP-70 payment requests and files or
...
voltaem.conf or as an argument to Voltaem-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside Voltaem
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `voltaem.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on Voltaem Core since
...
to support Voltaem parametrization.
...
```
voltaem-cli help getblockstats
```
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/voltaem/ltc-scrypt
