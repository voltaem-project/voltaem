Voltaem Core version 1.14.6 is now available from:

  <https://github.com/voltaem/voltaem/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All Voltaem Core users - miners, services,
...
Voltaem network from 1 VAEM to 0.01 VAEM. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 VAEM as
   change if possible.
2. If after signing there is more than 0.01 VAEM left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 VAEM, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 VAEM will not be accepted as valid by default.
...
```
voltaem-cli addnode <address> remove
```
...
```
voltaem-cli liststucktransactions
```
...
```
voltaem-cli setmaxconnections <desired_amount>
```
...
```
voltaem-cli rescan <start_block>
```
...
efforts to make Voltaem more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by Voltaem Core.
