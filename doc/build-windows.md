...
Below are some notes on how to build Voltaem Core for Windows.
...
To build Voltaem from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Voltaem GitHub repository](https://github.com/voltaem/voltaem/)).
...
Note that for WSL v1 the Voltaem Core source path MUST be somewhere in the default mount file system, for
example /usr/src/voltaem, AND not under, for example, /mnt/d/voltaem.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Voltaem's dependencies):
...
way. This will install to `c:\workspace\voltaem`, for example:

    make install DESTDIR=/mnt/c/workspace/voltaem
